...
import { FaArrowLeft, FaChartLine, FaUsers, FaMoneyBillWave, FaStar, FaMobile, FaDesktop } from 'react-icons/fa';
...
export default async function AdminAnalyticsPage() {
  ...
  const totalBookings = packageBookings.length + transferBookings.length + dailyTourBookings.length;

  // Calculate confirmed bookings (conversion)
  const confirmedPackages = packageBookings.filter(b => b.status === 'CONFIRMED' || b.status === 'COMPLETED').length;
  const confirmedTransfers = transferBookings.filter(b => b.status === 'CONFIRMED' || b.status === 'COMPLETED').length;
  const confirmedDailyTours = dailyTourBookings.filter(b => b.status === 'CONFIRMED' || b.status === 'COMPLETED').length;
  const totalConfirmed = confirmedPackages + confirmedTransfers + confirmedDailyTours;

  // Conversion rate (confirmed / total)
  const conversionRate = totalBookings > 0 ? ((totalConfirmed / totalBookings) * 100).toFixed(1) : '0';

  // Calculate total revenue (confirmed bookings only, in EUR)
  ...
  const dailyTourRevenue = dailyTourBookings
    .filter(b => b.status === 'CONFIRMED' || b.status === 'COMPLETED')
    .reduce((sum, b) => sum + b.totalPrice, 0);
  ...
  // Most popular daily tours (top 5)
  const tourCounts: Record<string, number> = {};
  dailyTourBookings.forEach(b => {
  ...
  // Most popular transfer routes (top 5)
  ...
  const b2cPackages = packageBookings.filter(b => !b.agentId).length;
  const b2bPackages = packageBookings.filter(b => b.agentId).length;
  const b2cTransfers = transferBookings.filter(b => !b.agentId).length;
  const b2bTransfers = transferBookings.filter(b => b.agentId).length;
  const b2cDailyTours = dailyTourBookings.filter(b => !b.agentId).length;
  const b2bDailyTours = dailyTourBookings.filter(b => b.agentId).length;
  ...
  const monthlyData = Array.from({ length: 6 }, (_, i) => {
    ...
    const monthPackages = packageBookings.filter(b =>
      b.createdAt >= monthStart && b.createdAt <= monthEnd
    ).length;
    const monthTransfers = transferBookings.filter(b =>
      b.createdAt >= monthStart && b.createdAt <= monthEnd
    ).length;
    const monthDailyTours = dailyTourBookings.filter(b =>
      b.createdAt >= monthStart && b.createdAt <= monthEnd
    ).length;
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">{totalConfirmed} confirmed</p>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold text-gray-900">{packageBookings.length}</span>
              </div>
              <div className="flex justify-between items-center">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">Total</span>
                <span className="font-semibold text-gray-900">{transferBookings.length}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">Confirmed</span>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">Total</span>
                <span className="font-semibold text-gray-900">{dailyTourBookings.length}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">Confirmed</span>
