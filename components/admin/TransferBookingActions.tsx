...
interface TransferBookingActionsProps {
  bookingId: string;
  currentStatus: string;
  customerPhone?: string | null;
}

export default function TransferBookingActions({ bookingId, currentStatus, customerPhone }: TransferBookingActionsProps) {
  const router = useRouter();
  const [updating, setUpdating] = useState(false);
  const [error, setError] = useState('');

  const updateStatus = async (newStatus: string) => {
    if (!confirm(`Are you sure you want to mark this transfer booking as ${newStatus}?`)) {
      return;
    }
    ...
    try {
      const res = await fetch(`/api/admin/bookings/transfers/${bookingId}`, {
        method: 'PATCH',
      ...
      if (res.ok) {
        ...
        setError(data.error || 'Failed to update transfer booking');
      }
    } catch {
    ...
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="space-y-3">
        {currentStatus === 'PENDING' && (
          <>
            ...
            >
              ...
              {updating ? 'Updating...' : 'Confirm Transfer'}
            </button>
            ...
            >
              ...
              {updating ? 'Updating...' : 'Reject Transfer'}
            </button>
          </>
        )}
        ...
        {currentStatus === 'CONFIRMED' && (
          <>
            ...
            >
              <FaTimes className="mr-2" />
              {updating ? 'Updating...' : 'Cancel Transfer'}
            </button>
          </>
        )}
      ...
      {currentStatus === 'CANCELLED' && (
        <div className="mt-4 text-center text-sm text-gray-500">
          This transfer booking has been cancelled
        </div>
      )}
